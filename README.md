...
| `anthill.html` | «Муравейник» — мессенджер со сквозным шифрованием (ЛС, группы, профили) |
...
## Муравейник: как устроено шифрование

- Ключи пользователя: ECDH P-256 (шифрование) + ECDSA P-256 (подпись). Приватные ключи — в IndexedDB как **неэкспортируемые** `CryptoKey`.
- Каждое сообщение: свой AES-256-GCM ключ + одноразовая эфемерная пара ECDH; ключ заворачивается для каждого участника через HKDF; всё подписано отправителем.
- Бэкап ключей шифруется **кодом восстановления** (160 бит, PBKDF2 600k). Код знает только пользователь.
- Ключи собеседников закрепляются при первом контакте; смена ключа → предупреждение и блокировка отправки в ЛС до подтверждения. Для сверки — код безопасности из 60 цифр.
- Картинки (аватары, фото) пересжимаются через canvas — EXIF/GPS не уходят.
- Найти пользователя можно только по точному `@нику`; списка пользователей нет.
- Доступ — только аккаунты email+пароль (без подтверждения почты; от спама стоит включить App Check).

**Ограничения (честно):** сервер видит метаданные (кто с кем и когда переписывается); нет полной прямой секретности (Double Ratchet) — утечка ключа получателя раскрывает адресованные ему сообщения.
